<template>
  <Card class="pt-0">
    <CardHeader class="flex items-center gap-2 py-4 pb-4! space-y-0 border-b dark:border-b-default sm:flex-row">
      ...
      <!-- <Select v-model="timeRange">
        <SelectTrigger
          class="hidden w-[160px] mr-9 rounded-lg sm:ml-auto sm:flex"
    ...
    <CardContent class="px-2 pt-2 pb-2 sm:px-6 sm:pt-6">
      <ChartContainer :config="chartConfig" class="aspect-auto h-[180px] w-full" :cursor="false">
        <VisXYContainer :data="filterRange" :svg-defs="svgDefs" :margin="{ left: -40 }" :y-domain="[0, 1200]">
          <VisArea :x="(d: Data) => d.date" :y="[(d: Data) => d.mobile, (d: Data) => d.desktop]" :color="(_d: Data, i: number) => ['url(#fillMobile)', 'url(#fillDesktop)'][i]" :opacity="0.6" />
          <VisLine :x="(d: Data) => d.date" :y="[(d: Data) => d.mobile, (d: Data) => d.mobile + d.desktop]" :color="(_d: Data, i: number) => [chartConfig.mobile.color, chartConfig.desktop.color][i]" :line-width="1" />
          <VisAxis
            type="x"
            :x="(d: Data) => d.date"
            :tick-line="false"
...
import { ChartContainer, ChartCrosshair, ChartLegendContent, ChartTooltip, ChartTooltipContent, componentToString } from '@/components/shadcn-ui/chart';
