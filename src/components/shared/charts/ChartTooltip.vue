<template>
  <Card>
    ...
    <CardContent>
      <ChartContainer :config="chartConfig" class="aspect-auto h-[220px] w-full">
        <VisXYContainer :data="chartData" :padding="{ top: 10, bottom: 10, left: 10, right: 10 }">
          <VisStackedBar :x="(d: Data) => d.date" :y="[(d: Data) => d.running, (d: Data) => d.swimming]" :color="[chartConfig.running.color, chartConfig.swimming.color]" :rounded-corners="4" :bar-padding="0.1" />
          <VisAxis
            type="x"
            :x="(d: Data) => d.date"
            :tick-line="false"
